# Changelog

## 1.0.0 — 2026-09-25

Phase 9 of the 2026-09-24 expansion plan.

- Members as decided in D19: `documentation-architect` (the checker, through a docs build that fails), `technical-writer`, `api-designer`. No developer advocate.
- No software architect: per language (ADR 0015).
- Two integrations: `context7-mcp`, `github-mcp`.
- `not_for` names the small, sequential and same-file work where more agents measurably do worse, and the README cites MAST and Kim et al. for why the crew stays small.
- `tier: community`, `provenance: generated` (ADR 0011).
